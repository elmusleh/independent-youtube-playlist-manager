<script lang="ts">
  ...
  $: displayedPlaylists = (() => {
    let result = allPlaylists.filter((p) => {
      if ($playlistsSearch && !p.title.toLowerCase().includes($playlistsSearch.toLowerCase())) {
        return false;
      }
...
<main class="flex flex-col gap-lg p-6">
  ...
  {:else if displayedPlaylists.length === 0}
    <!-- Empty State -->
    <section class="empty-state flex flex-col gap-md items-center justify-center py-12">
      <Fa icon={faSearch} size="3x" class="text-tertiary" />
      <div class="text-center">
        {#if $playlistsSearch}
          <h2 class="text-lg font-semibold">No playlists match your search</h2>
          <p class="text-secondary">Try different keywords or clear your search</p>
        {:else if activeChip === "Managed"}
          <h2 class="text-lg font-semibold">No managed playlists yet</h2>
          <p class="text-secondary">Adopt or create playlists to get started</p>
        {:else}
          <h2 class="text-lg font-semibold">No playlists found</h2>
          <p class="text-secondary">Create your first playlist to begin</p>
        {/if}
      </div>
    </section>
  {:else}
    ...
    <CardGrid minWidth="md" gap="lg" class="container-query">
      {#each displayedPlaylists as p (p.id)}
        <Card interactive elevated glow onClick={() => openInEditor(p)}>
          <!-- Thumbnail Section -->
          <div class="relative aspect-video rounded-lg overflow-hidden bg-surface-200 group">
            {#if p.thumbnailUrl}
              <img
                alt={p.title}
                src={p.thumbnailUrl}
              ...
              />
            {:else}
              <div class="flex items-center justify-center text-tertiary h-full" aria-hidden="true">
          ...
          <div class="flex flex-col gap-sm flex-1 mt-4">
            ...
            <p class="text-sm text-secondary">
              Created {new Date(p.timestamp * 1000).toLocaleDateString()}
            </p>
            ...
            <div class="flex gap-sm mt-auto pt-2 flex-wrap">
              ...
              {#if !p.isTagged && !p.isLocal && signedIn}
                <button
                  ...
                  on:click|stopPropagation={() => requestAdopt(p)}
                  disabled={adoptingId === p.id}
                  aria-label={adoptingId === p.id ? "Adopting..." : "Adopt this playlist"}
                >
                  {adoptingId === p.id ? "Adopting…" : "Adopt"}
                </button>
              {/if}
              ...
              {#if p.isLocal && signedIn}
                <button
                  class="btn btn-secondary flex-1"
                  on:click|stopPropagation={() => requestSync(p)}
                  disabled={syncingId === p.id}
                  aria-label={syncingId === p.id ? "Syncing..." : "Sync to YouTube"}
                >
                  <Fa icon={faCloud} fw />
  ...
  {#if !signedIn && allPlaylists.length > 0}
    <div class="mt-6 rounded-lg border border-warning bg-warning-light p-4 flex gap-3 items-start">
      <Fa icon={faCloud} fw size="lg" class="text-warning flex-shrink-0 mt-1" />
      <div class="flex-1">
        <p class="text-sm text-warning font-semibold">Showing offline playlists</p>
        <p class="text-xs text-secondary mt-1">
          <button class="text-primary font-semibold hover:underline" on:click={triggerSignIn}>
            Sign in to YouTube
          </button>
