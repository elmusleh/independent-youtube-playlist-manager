...
export class StorageManager {
  ...
  async init() {
    try {
      ...
      if (handle) {
        const hasPermission = await this.verifyPermission(handle as FileSystemDirectoryHandle, false);
        if (hasPermission) {
  ...
  async verifyPermission(fileHandle: FileSystemDirectoryHandle, requestIfMissing = false): Promise<boolean> {
