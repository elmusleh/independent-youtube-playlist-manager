...
<div 
  class="list-row {selected ? 'selected' : ''} {isDragging ? 'dragging' : ''} {isDragOver ? 'drag-over' : ''}"
  draggable="true"
...
>
    ...
    <div class="playlist-title-row">
      ...
      {#if !isLocal}
        <div class="privacy-badge {privacy}">
          <Fa icon={privacy === 'public' ? faEarthAmericas : (privacy === 'unlisted' ? faLockOpen : faLock)} fw />
          <span>{privacy}</span>
        </div>
      {/if}
    </div>
  ...
  <div class="row-actions">
    <SimpleButton 
      iconOnly 
      secondary 
      onclick={(e) => { e.stopPropagation(); onEdit(playlist.id); }} 
      title="Edit playlist"
    >
      <Fa icon={faPencil} fw />
    </SimpleButton>
    ...
    {#if !isLocal}
      {#if onOpenYoutube}
        <SimpleButton 
          iconOnly 
          secondary 
          onclick={(e) => { e.stopPropagation(); onOpenYoutube(playlist); }} 
      ...
      {#if onSaveOffline}
        <SimpleButton 
          iconOnly 
          secondary 
          onclick={(e) => { e.stopPropagation(); onSaveOffline(playlist.id); }} 
...
<style>
  ...
  .privacy-badge.public { background: rgba(76, 175, 80, 0.1); color: #4caf50; }
  .privacy-badge.unlisted { background: rgba(158, 158, 158, 0.1); color: #9e9e9e; }
  .privacy-badge.private { background: rgba(255, 152, 0, 0.1); color: #ff9800; }
  ...
  .system-badge { background: #607d8b; color: #fff; }
  .liked-badge { background: #f44336; color: #fff; }
  .uploads-badge { background: #2196f3; color: #fff; }
</style>
