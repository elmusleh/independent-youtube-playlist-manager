...
*   **Zero-Quota First Resolution**: Resolves video title, channels, and statuses without using YouTube Data API quota:
    1.  *Local IndexedDB Cache*: Instant lookup hits.
    2.  *Innertube Client Engine*: Modern `MWEB`/`WEB` player queries that bypass bot blocks.
    3.  *Embed Page Headless Scraper*: Extracts JSON player state from embedded page HTML.
    4.  *Official YouTube oEmbed*: Zero-quota title and channel details.
    5.  *Piped & Invidious Instance Fallbacks*: Distributed fallback endpoints.
