...
- [x] Build dedicated zero-library native IndexedDB wrapper (`yph_metadata_db_v2` in `apps/browser-extension/apps/browser-extension/playlist-editor/apps/browser-extension/services/db-service.ts`).
...
- [x] Initialize Next.js 15 App Router with Tailwind CSS and dark mode theme in `apps/web-portal/`.
...
- [x] Create production PostgreSQL schema with Row-Level Security (`apps/web-portal/supabase/schema.sql`).
- [x] Implement Supabase server and browser client integration with graceful offline fallback (`apps/web-portal/apps/browser-extension/lib/supabase.ts`).
...
- [x] **Supabase Live Deployment:**
  ...
  - [x] Set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` in `apps/web-portal/.env.local`.
...
- [ ] **Web SaaS Portal Deployment:** (requires Vercel/Cloudflare account credentials)
  - [x] Add `apps/web-portal/vercel.json` (Next.js framework, build/install commands) — deploy-ready config committed.
  - [ ] Connect `apps/web-portal/` repository to **Vercel** or **Cloudflare Pages** (manual: needs owner login).
